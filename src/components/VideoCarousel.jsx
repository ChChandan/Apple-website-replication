  ...
  return (
    <>
      ...
      <div className="relative flex-center mt-10">
        <div className="flex-center py-5 px-10 bg-gray-300 blackdrop-blur rounded-full">
            {videoRef.current.map((_,i)=>(
                <span key={i} ref={(el)=>(videoDivRef.current[i] = el)} className="mx-2 w-3 h-3 bg-grey-200 rounded-full relative cursor-pointer">
                <span className="absolute h-full w-full rounded-full" ef={(el)=>(videoSpanRef.current[i] = el)}/>
